...
namespace vkInit {
    ...
    SwapChainSupportDetails query_swapchain_support(vk::PhysicalDevice device, vk::SurfaceKHR vulkanSurface, bool debug) {

		SwapChainSupportDetails support;

		
		support.capabilities = device.getSurfaceCapabilitiesKHR(vulkanSurface);
		...
		*/
		...
		support.formats = device.getSurfaceFormatsKHR(vulkanSurface);
		...
		support.presentModes = device.getSurfacePresentModesKHR(vulkanSurface);
	...
	*/
	vk::PresentModeKHR choose_swapchain_present_mode(std::vector<vk::PresentModeKHR> presentModes) {
		...
		//Fifo is required so default to that if others fail
	...
	*/
	...
	//can use pointers here to have less args but avoided to prevent cirular imports
	SwapChainBundle create_swapchain(vk::Device logicalDevice, vk::PhysicalDevice physicalDevice, vk::SurfaceKHR vulkanSurface, int width, int height, bool debug) {

		SwapChainSupportDetails support = query_swapchain_support(physicalDevice, vulkanSurface, debug);
      ...
      VULKAN_HPP_NAMESPACE::SwapchainKHR   oldSwapchain_ = {} ) VULKAN_HPP_NOEXCEPT
		*/
		vk::SwapchainCreateInfoKHR createInfo = vk::SwapchainCreateInfoKHR(
			vk::SwapchainCreateFlagsKHR(), vulkanSurface, imageCount, format.format, format.colorSpace,
			extent, 1, vk::ImageUsageFlagBits::eColorAttachment
		);


		vkUtil::QueueFamilyIndices indices = vkUtil::findQueueFamilies(physicalDevice, vulkanSurface, debug);
